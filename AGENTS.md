...
The repository MUST maintain these files:
...
Contains stable long-term project memory:
...
Contains compressed operational state:
...
This file MUST remain concise and operational.
...
Defines the active scoped task.
...
The agent MUST NOT work outside this scope.

---

### RESTART_PROMPT.md

Contains the minimal bootstrap instructions required to resume work in a new session.

---

# Checkpoint Protocol

The agent MUST checkpoint:
...
* before large refactors
  OR
* before context windows become large

Checkpointing means:

1. Update SESSION_STATE.yaml
2. Update RESTART_PROMPT.md
3. Compress execution state
4. Remove obsolete details
5. Preserve only actionable engineering state

---

# Compression Rules

Operational summaries MUST:
...
* TLS timeout during SSL handshake

Bad:

"I spent time debugging SSL behavior and think the issue may..."
...
2. Verify task boundaries
3. Verify files in scope
...
The restart prompt MUST allow another agent session to resume work immediately with minimal context.
