#!/usr/bin/env python3
"""Fix EW001 - use amovie filter to force audio duration."""
import subprocess
...
print("Creating scene videos with CORRECT audio using amovie filter...\n")
...
for i in range(1, 14):
    ...
    if vo_path.exists():
        ...
        # Use amovie filter to load audio and loop it, then combine with video
        cmd = [
            'ffmpeg', '-y',
            '-loop', '1', '-i', str(img_path),
            '-i', str(ext_audio),
            '-vf', f'scale=1920:1080:force_original_aspect_ratio=decrease,crop=1920:1080',
            '-af', f'aloop=loop=-1:sleep=0,atrim=0:{dur}',  # Loop audio infinitely then trim to duration
            '-t', str(dur),
